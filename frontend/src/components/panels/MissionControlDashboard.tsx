...
export function MissionControlDashboard({ systems, bookings }: Props) {
  ...
  const utilizationBySystem = systems.map((system) => {
    ...
    const cpu = active.reduce((sum, booking) => sum + booking.req_cpu, 0)
    const gpu = active.reduce((sum, booking) => sum + booking.req_gpu, 0)
    const ram = active.reduce((sum, booking) => sum + booking.req_ram, 0)
    const vram = active.reduce((sum, booking) => sum + booking.req_vram, 0)
    ...
    return {
      ...
      ramPct,
      vramPct,
      pressurePct: Math.max(cpuPct, gpuPct, ramPct, vramPct),
    }
  })

  const highGpuAlerts = utilizationBySystem.filter((entry) => entry.gpuPct >= 80)

  const preemptionCountBySystem = systems.map((system) => ({
    system,
    preemptions: bookings.filter((booking) => booking.system_id === system.id && booking.status === "PREEMPTED").length,
  }))

  const repeatedPreemptionAlerts = preemptionCountBySystem.filter((entry) => entry.preemptions >= 3)

  const offlineAlerts = systems.filter((system) => {
    const status = (system as ComputeSystem & { status?: string }).status
  ...
  const heatmapCells = systems.flatMap((system) => {
    ...
    return Array.from({ length: columns }).map((_, hourOffset) => {
      const segmentStart = nowMs + hourOffset * hourMs
      const segmentEnd = segmentStart + hourMs
      ...
      const overlapping = bookings.filter((booking) => {
        ...
        return overlap(segmentStart, segmentEnd, bookingStart, bookingEnd)
      })

      const gpuLoad = overlapping.reduce((sum, booking) => sum + booking.req_gpu, 0)
  ...
  const pressureScore = utilizationBySystem.length
    ? Math.round(utilizationBySystem.reduce((sum, item) => sum + item.pressurePct, 0) / utilizationBySystem.length)
    : 0
      ...
      <CardContent>
        <Stack spacing={2}>
          ...
          <Stack direction="row" spacing={1} flexWrap="wrap">
            ...
            <Chip label={`Resource pressure: ${pressureScore}%`} variant="outlined" color={pressureScore >= 80 ? "error" : "default"} />
          </Stack>
          ...
          {highGpuAlerts.length ? (
            <Alert severity="warning">
              High GPU usage on: {highGpuAlerts.map((entry) => entry.system.name).join(", ")}
            </Alert>
          ) : null}

          {repeatedPreemptionAlerts.length ? (
            <Alert severity="warning">
              Repeated preemptions detected: {repeatedPreemptionAlerts.map((entry) => `${entry.system.name} (${entry.preemptions})`).join(", ")}
            </Alert>
          ) : null}

          {offlineAlerts.length ? (
            <Alert severity="error">Offline systems: {offlineAlerts.map((system) => system.name).join(", ")}</Alert>
          ) : (
            <Alert severity="info">No offline systems reported by current API payload.</Alert>
          )}

          <Divider />

          <Typography variant="subtitle2">Utilization Heatmap (Next 24h, GPU pressure)</Typography>
          <Box sx={{ display: "grid", gridTemplateColumns: `160px repeat(24, minmax(12px, 1fr))`, gap: 0.3, alignItems: "center" }}>
            {systems.map((system) => (
              <Box key={`row-${system.id}`} sx={{ display: "contents" }}>
                ...
                {heatmapCells
                  .filter((cell) => cell.systemId === system.id)
                  .map((cell) => (
                    <Box
